...
```{r}
freedom <- read_csv("Desktop/DataScience/math241/activities/freedom.csv")
```
...
freedom_ranked %>%
  filter(country == "Iran") %>% 
  ggplot(aes(x = year, y = civil_liberty))+geom_line()
```
...
freedom_ranked2 <- freedom_ranked %>% 
  filter(country == "Iran")

p <- ggplot(freedom_ranked2, aes(x = year, y = civil_liberty))+
  geom_line()

p_animate <- p +
  transition_reveal(year)

animate(p_animate, fps = 5, end_pause = 20)
```
