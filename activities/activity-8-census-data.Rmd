...
```{r, eval = F}
census_api_key("3579cb4ddc820f37984700778b812af510e15c1a", install = TRUE)
```
...
or_county <- or_county %>%
  mutate(percentage_white = (white_popE / total_popE) * 100)
```
...
# Assuming the dataset includes a column for county names or identifiers (e.g., COUNTYFP for FIPS code)
# Check unique values to confirm they all belong to Multnomah County
# The exact column name may vary; adjust as needed based on the glimpse() output
unique(ortracts$COUNTYFP)
```

It looks like there are census tracts as well.
...
wayne_values <- get_acs(geography = "tract", variables = "B25077_001", 
                        year = 2022, state = "MI", county = "Wayne", 
                        survey = "acs5", output = "wide")
```

```{r}
# For Wayne County
wayne_tracts <- tracts(state = "MI", county = "Wayne", cb = TRUE)

wayne_data <- left_join(wayne_values, wayne_tracts, by = c("GEOID" = "GEOID"))
```
...
wayne_data %>%
  st_as_sf() %>%
  ggplot() +
  geom_sf(aes(fill = B25077_001E), color = NA) +
  scale_fill_viridis_c(name = "Median Home Value", option = "plasma") + #color scale
  labs(title = "Median home values in Wayne County, MI", subtitle = "By Census Tract") +
  theme_void() +
